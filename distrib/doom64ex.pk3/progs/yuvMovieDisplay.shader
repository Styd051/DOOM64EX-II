...
def_sampler(2D, tSamp0, 0);
def_sampler(2D, tSamp1, 1);

#ifndef __ORBIS__
def_sampler(2D, tSamp2, 2);
#endif
...
shader_main(outPixel, outVertex, input)
{
    ...
    yuv.x = sampleLevelZero(tSamp0, inVar(input, out_texcoord)).r;
    
#ifndef __ORBIS__
    yuv.y = sampleLevelZero(tSamp1, inVar(input, out_texcoord)).r;
    yuv.z = sampleLevelZero(tSamp2, inVar(input, out_texcoord)).r;
#else
    yuv.yz = sampleLevelZero(tSamp1, inVar(input, out_texcoord)).rg;
#endif
