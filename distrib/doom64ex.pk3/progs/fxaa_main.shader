...
shader_main(outPixel, outVertex, input)
{
    ...
    vec3 rgbNW = sampleLevelZero(tBase, texcoord + (vec2(-1.0, -1.0) / frameBufSize)).xyz;
    vec3 rgbNE = sampleLevelZero(tBase, texcoord + (vec2( 1.0, -1.0) / frameBufSize)).xyz;
    vec3 rgbSW = sampleLevelZero(tBase, texcoord + (vec2(-1.0,  1.0) / frameBufSize)).xyz;
    vec3 rgbSE = sampleLevelZero(tBase, texcoord + (vec2( 1.0,  1.0) / frameBufSize)).xyz;
    vec3 rgbM  = sampleLevelZero(tBase, texcoord).xyz;
    ...
    vec3 rgbA = rcp(2.0) * (sampleLevelZero(tBase, texcoord + dir * (1.0/3.0 - 0.5)).xyz +
                             sampleLevelZero(tBase, texcoord + dir * (2.0/3.0 - 0.5)).xyz);
    
    vec3 rgbB = rgbA * rcp(2.0) + rcp(4.0) * (sampleLevelZero(tBase, texcoord + dir * (0.0/3.0 - 0.5)).xyz +
                                                sampleLevelZero(tBase, texcoord + dir * (3.0/3.0 - 0.5)).xyz);
...
shader_main(outPixel, outVertex, input)
{
    ...
    vec3 rgbN = sampleLevelZero(tBase, pos + vec2( 0.0, -1.0) * rcpFrame).xyz;
    vec3 rgbW = sampleLevelZero(tBase, pos + vec2(-1.0,  0.0) * rcpFrame).xyz;
    vec3 rgbM = sampleLevelZero(tBase, pos + vec2( 0.0,  0.0) * rcpFrame).xyz;
    vec3 rgbE = sampleLevelZero(tBase, pos + vec2( 1.0,  0.0) * rcpFrame).xyz;
    vec3 rgbS = sampleLevelZero(tBase, pos + vec2( 0.0,  1.0) * rcpFrame).xyz;
    ...
    vec3 rgbNW = sampleLevelZero(tBase, pos + vec2(-1.0, -1.0) * rcpFrame).xyz;
    vec3 rgbNE = sampleLevelZero(tBase, pos + vec2( 1.0, -1.0) * rcpFrame).xyz;
    vec3 rgbSW = sampleLevelZero(tBase, pos + vec2(-1.0,  1.0) * rcpFrame).xyz;
    vec3 rgbSE = sampleLevelZero(tBase, pos + vec2( 1.0,  1.0) * rcpFrame).xyz;
    ...
    {
        if(!doneN) lumaEndN = FxaaLuma(sampleLevelZero(tBase, posN.xy).xyz);
        if(!doneP) lumaEndP = FxaaLuma(sampleLevelZero(tBase, posP.xy).xyz);
    ...
    vec3 rgbF = sampleLevelZero(tBase, posF).xyz;
