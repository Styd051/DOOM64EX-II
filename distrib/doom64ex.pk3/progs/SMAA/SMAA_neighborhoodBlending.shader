...
shader_main(outPixel, outVertex, input)
{
    ...
    {
        color = sampleLevelZero(tDest, inVar(input, out_texcoord));
    }
    else
    {
        ...
        // neighbor:
        color = blendingWeight.x * sampleLevelZero(tDest, blendingCoord.xy);
        color += blendingWeight.y * sampleLevelZero(tDest, blendingCoord.zw);
    }
