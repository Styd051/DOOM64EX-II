...
shader_pixel(outPixel, outVertex, input)
{
    ...
    float depth = RGBAToDepth(load(tDepth, vCoord, 0));
    
    float fD00 = RGBAToDepth(load(tDepth, vCoord+vOffset.xy, 0));
    float fD01 = RGBAToDepth(load(tDepth, vCoord+vOffset.xw, 0));
    float fD11 = RGBAToDepth(load(tDepth, vCoord+vOffset.zw, 0));
    float fD10 = RGBAToDepth(load(tDepth, vCoord+vOffset.zy, 0));
