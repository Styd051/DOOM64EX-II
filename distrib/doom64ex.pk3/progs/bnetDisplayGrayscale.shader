...
shader_main(outPixel, outVertex, input)
{
    ...
    vec4 rgb = sampleLevelZero(tBase, inVar(input, out_texcoord));
    
    float fWeight = (0.2126 * rgb.r + 0.7512 * rgb.g + 0.0722 * rgb.b);
    outVarFragment(output, fragment) = vec4(fWeight, fWeight, fWeight, rgb.a) * inVar(input, out_color);
    outReturn(output)
}
